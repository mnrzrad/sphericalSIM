simu#' Run Single Simulation for Spherical Single-Index Model with Group Lasso
#'
...
#'
#' @param results List of simulation results from \code{\link{run_simulation_study}}
#'   or multiple calls to \code{\link{run_single_simulation}}
#'
#' @details
...
#
#   metrics <- data.frame(
#     sim_id = sapply(results, function(x) x$sim_id),
#     n_selected = sapply(results, function(x) x$n_selected),
#     TPR = sapply(results, function(x) x$TPR),
#     FPR = sapply(results, function(x) x$FPR),
#     beta_l2_error = sapply(results, function(x) x$beta_l2_error),
#     angular_error = sapply(results, function(x) x$angular_error),
#     link_mse = sapply(results, function(x) x$link_mse),
#     pred_error = sapply(results, function(x) x$pred_error),
#     best_lambda = sapply(results, function(x) x$best_lambda),
#     best_gamma = sapply(results, function(x) x$best_gamma),
...
#     Metric = c("n_selected", "TPR", "FPR", "beta_l2_error",
#                "angular_error", "link_mse", "pred_error"),
#     Mean = c(mean(metrics$n_selected), mean(metrics$TPR), mean(metrics$FPR),
#              mean(metrics$beta_l2_error), mean(metrics$angular_error),
#              mean(metrics$link_mse), mean(metrics$pred_error)),
#     SD = c(sd(metrics$n_selected), sd(metrics$TPR), sd(metrics$FPR),
#            sd(metrics$beta_l2_error), sd(metrics$angular_error),
#            sd(metrics$link_mse), sd(metrics$pred_error)),
#     Median = c(median(metrics$n_selected), median(metrics$TPR), median(metrics$FPR),
#                median(metrics$beta_l2_error), median(metrics$angular_error),
#                median(metrics$link_mse), median(metrics$pred_error))
#   )
#
#   print(summary_stats, digits = 4)
#
#   cat(sprintf("\nConvergence: %.1f%%\n", 100 * mean(metrics$converged)))
#   cat(sprintf("Mean iterations: %.1f (SD: %.1f)\n",
#               mean(metrics$n_iter), sd(metrics$n_iter)))
#
#   list(metrics = metrics, summary = summary_stats)
# }
#' Summarize Simulation Study Results (Updated for Complete Metrics)
#'
